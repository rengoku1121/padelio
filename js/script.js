...
(() => {
  ...
  const generateRound = async () => {
    ...
    if (mode === 'mexicano') {
      ...
      let matches = [];
      if (maxCourts > 0) {
        const builder = window.Padelio?.buildMexicanoMatches;
        if (typeof builder === 'function') {
          ...
          toast('Mexicano pairing module missing — reload the page.');
        }
      }
      roundData = { round: roundNo, matches };
      rounds.push(roundData);
    ...
    if (mode === 'mixmex') {
      ...
      let matches = [];
      if (maxCourts > 0) {
        const builder = window.Padelio?.buildMixMexicanoMatches;
        if (typeof builder === 'function') {
          matches = builder(
            playersFull, courts, rounds, roundNo, state.currentTournament
          ) || [];
        } else {
          toast('Mix Mexicano pairing module missing — reload the page.');
        }
      }
      roundData = { round: roundNo, matches };
      rounds.push(roundData);
    ...
    if (mode === 'fixed' || mode === 'fixedmex') {
      ...
      if (maxCourts > 0) {
        ...
        if (mode === 'fixedmex') {
          const builder = window.Padelio?.buildFixedPairsMexicanoMatches;
          if (typeof builder === 'function') {
            matches = builder(
              allPairObjs, courts, rounds, roundNo, state.currentTournament, playersFull
            ) || [];
          } else {
            toast('Fixed Mexicano pairing module missing — reload the page.');
          }
        } else {
